...
where s.product_ID = coi.product_ID) as 'SUBTOTAL',
# Discount [EUR] = discount [%] / 100 * subtotal
(select sum(coi.discount_percent/100 * s.unit_price * coi.quantity)
from  stock s, customer_order_item coi 
where s.product_ID = coi.product_ID) as 'DISCOUNT',
# Tax rate
(select distinct (co.tax_rate_percent/100) 
from customer_order_item coi , customer_order co 
where coi.customer_order_and_invoice_ID = co.customer_order_and_invoice_ID and co.customer_order_and_invoice_ID = 1) 
as '(TAX RATE)',
# Tax
((select distinct (co.tax_rate_percent/100) 
from customer_order_item coi , customer_order co 
where coi.customer_order_and_invoice_ID = co.customer_order_and_invoice_ID and co.customer_order_and_invoice_ID = 1)
* (select (sum(s.unit_price * coi.quantity - coi.discount_percent/100 * s.unit_price * coi.quantity)) 
from  stock s, customer_order_item coi 
